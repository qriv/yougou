 ...
 * @author Tan Chunmao
 */
public class SettingsAccountActivity extends ActivityBase {
    ...
    @InjectView
    private View layout_avatar, layout_nickname, layout_address, layout_login_password, layout_pay_password, layout_mobile;
    @InjectView
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!TextUtils.isEmpty(localAvatarPath)) {
            ...
            if (!TextUtils.isEmpty(avatarPath)) {
                loadAvatar(avatar, avatarPath);
            } else {
                avatar.setImageResource(R.drawable.ui_default_avatar);
            }
        }
        String name = MallApplication.user.getNickname();
        ...
        layout_login_password.setOnClickListener(onClickListener);
        layout_pay_password.setOnClickListener(onClickListener);
        layout_mobile.setOnClickListener(onClickListener);
    }
    ...
    private View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (v.equals(layout_avatar)) {
                ...
                setAddress();
            } else if (v.equals(layout_login_password)) {
                changeLoginPassword();
            } else if (v.equals(layout_pay_password)) {
                setPayPassword();
            } else if (v.equals(layout_mobile)) {
                changeMobile();
            }
        }
    };
    ...
    private void changeLoginPassword() {
        go.name(ChangeLoginPasswordActivity.class).go();
    }

    private void setPayPassword() {
        go.name(SetPayPasswordActivity.class).go();
    }

    private void changeMobile() {
        go.name(ChangeMobileActivity.class).go();
    }
    ...
    private void setAddress() {
        go.name(AddressListActivity.class).go();
    }
    ...
    private void changeAvatar() {
        ...
        execute(new Request(Urls.MEMBER_PROFILE_UPDATE).setHttpBody(body),
    ...
    private void changeNickname(final String nicknameText) {
        showProgressDialog();
        execute(new Request(Urls.MEMBER_PROFILE_UPDATE).addUrlParam("nickname", nicknameText),
                new HttpResultHandler() {
                    @Override
                    protected void onSuccess(ApiResult apiResult) {
                        dismissProgressDialog();
