...
import com.gsh.app.client.mall.activity.ImgAction;
 ...
 * @author Tan Chunmao
 */
public class BasketCommentActivity extends ActivityBase implements View.OnClickListener, View.OnLongClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        comboOrderDetails = (ComboOrderDetails) getIntent().getSerializableExtra(ComboOrderDetails.class.getName());
    ...
    private void initViews() {
        loadImage(pic, comboOrderDetails.mainPicturePath);
        title.setText(comboOrderDetails.comboName);
...
//        fillPhotos();
    ...
    private void comment(String text) {
        if (MallApplication.test) {
            comboOrderDetails.comboId = 1;
        }
        execute(new Request(Urls.COMBO_VOTE).addUrlParam("comboId", String.valueOf(comboOrderDetails.comboId)).addUrlParam("content", text).addUrlParam("star", String.valueOf(rate)).addUrlParam("orderId", String.valueOf(comboOrderDetails.orderId)),
    ...
    private void submit() {
        ...
        if (TextUtils.isEmpty(text)) {
            ...
            showProgressDialog();
            execute(new Request(Urls.GOODS_COMMENT).addUrlParam("goodsId", String.valueOf(1)).addUrlParam("text", text).addUrlParam("rate", String.valueOf(rate)).addUrlParam("orderId", "").setHttpBody(body),
                    new HttpResultHandler() {
                        @Override
                        protected void onSuccess(ApiResult apiResult) {
                            dismissProgressDialog();
